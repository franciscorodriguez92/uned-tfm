...
\label{sec:Ejemplo_seccion}
\noindent AWS es una creciente unidad dentro la compañía Amazon.com que ofrece una importante variedad de soluciones de Cloud Computing tanto a PYMES como a grandes empresas a través de su infraestructura interna, siendo la marca más utilizada actualmente en el mercado de la nube con casi un 40\% de cuota de mercado \cite{AWScuota}. Amazon ofrece unos servicios en la nube pública mediante una tarificación de precios en función del tiempo de uso, anchos de banda consumidos, etc. Por lo tanto, su gran ventaja competitiva es ofrecer unos recursos de infraestructura y plataforma poco asumibles a la mayoría de empresas para el periodo que se requiera. 

Los clientes de AWS tan sólo deben pagar lo que usen del servicio, de esta manera, se puede acceder a recursos de almacenamiento, procesado o hardware por un periodo determinado, prescindiendo así de importantes inversiones en infraestructura. De este modo, estos servicios se adapta con total flexibilidad y escalabilidad a las necesidades del usuario.
...
\begin{itemize}
	...
	\item AWS Lambda: se trata de un servicio de computación sin servidor. Este servicio permite ejecutar código sin aprovisionar ni administrar servidores, pagando únicamente por el tiempo de cómputo que se consuma. De este modo, este servicio permite que sea AWS quien se encargue de la administración de las máquinas y el usuario únicamente trabaje en el código que se ejecuta.
\end{itemize}
...
\begin{itemize}
	...
	\item Etiquetado sintáctico: Proceso en el que se busca sobre el espacio de todas las posibles combinaciones de las reglas gramaticales definidas para encontrar la estructura sintáctica de una oración.
	\item Etiquetado morfológico: En este proceso se le asigna a cada palabra su categoría gramatical. Normalmente, se utilizan ocho etiquetas distintas en la mayoría de los idiomas utilizados en Europa: nombre, verbo, pronombre, preposición, adverbio, conjunción, partícula y articulo.
\end{itemize}
...
\begin{itemize}
	\itemsep0em 
	\item Freeling \cite{carreras04}: Es una librería que soporta el lenguaje español y se utiliza en \cite{Frenda2018}. Pese a que tiene muchas de las características que se necesitan, tiene una menor comunidad y está menos extendido que otras herramientas que veremos a continuación.
	\item Stanford Parser \cite{StanfordParser}: Se trata de una librería desarrollada por el grupo de trabajo de NLP de la universidad de Stanford. Cuenta con gran cantidad de componentes donde destacan los módulos de detección de entidades nombradas y etiquetado sintáctico disponibles para siete lenguajes distintos.
	\item TweetNLP \cite{TweetNLP}: Es una librería desarrollada específicamente para el procesado de tweets. Pese a que su uso no está aún muy extendido, implementa funcionalidades como el agrupamiento de tweets o un tokenizador específico. Sin embargo, el mantenimiento del proyecto no es tan activo como el de otras herramientas que se citan a continuación.
	\item Spacy \cite{Spacy}: Esta librería es la más reciente de todas las estudiadas y su uso está extendido entre la comunidad científica \cite{Waseem2016}. Permite aplicar un gran número de técnicas de procesado de un modo eficiente y con soporte para más de 49 lenguajes.
	\item NLTK \cite{NLTKweb}: Se trata de la librería más extendida actualmente para el preprocesamiento, se utiliza en \cite{Zimmerman2018,Davidson2017,Frenda2018}.
\end{itemize}

De todas las herramientas listadas, se ha optado por la librería NLTK. Se trata de una librería muy extendida que cuenta con una gran comunidad y permite un desarrollo muy ágil. Además, algunas librerías como Freeling o Stanford Parser requieren varias dependencias para poder ser utilizadas.
...
\label{sec:Ejemplo_seccion}
\noindent NLTK \cite{NLTKweb} es una librería que define una infraestructura en la que crear programas para el procesado del lenguaje natural (NLP, ``\textit{Natural language processing}'') en Python. Provee la estructura básica para representar datos relevantes para el procesado del lenguaje natural, interfaces para realizar tareas como el etiquetado del discurso (POS, ``part-of-speech tagging''), etiquetado sintáctico y clasificación de texto.
...
En ``scikit-learn'' es posible realizar selección de modelos mediante el meta-estimador ``GridSearchCV''. Este método toma como entrada un ``estimator'' cuyos parámetros de entrada deben ser optimizados. Para ello, se definen todos los valores que se deben tener en cuenta en el proceso para cada parámetro de entrada.
