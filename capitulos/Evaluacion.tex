...
\begin{itemize}
	...
	\item Precisión (precision): representa la fracción de asignaciones correctas frente al total de asignaciones positivas realizadas para esa clase. Es decir, realiza una medida de la tasa de acierto para un valor de la clase.
...
Para el presente trabajo, se utilizará como conjunto de evaluación del sistema de clasificación de contenido machista el corpus MeTwo presentado en el capítulo 4. Se trata de un corpus compuesto por 3600 tweets recopilados mediante el uso de expresiones que pueden conllevar actitudes machistas. 
...
La tabla 6.1 muestra los resultados promedio de exactitud (\textit{Accuracy}), medida F1, cobertura (\textit{Recall}) y precisión (precision). Con el método de evaluación descrito para el primer experimento, el algoritmo \textit{Random Forest} alcanza una mayor tasa de acierto y precisión, mientras que la regresión logística alcanza los mejores resultados para la medida F1 y \textit{recall}. 
...
En el caso de la línea base basada en el clasificador sencillo de la clase mayoritaria, sí se pueden observar grandes diferencias en las métricas de calidad. Esto se produce porque, una solución basada en un sistema informativo, aprenderá todos los valores de la clase, mientras que un sistema no informativo solo indicará una clase como su salida. De este modo, que cualquiera de las soluciones será mucho más adecuada que un clasificador basado en una única regla sencilla.
